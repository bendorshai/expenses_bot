...
class Categorizer:
    ...
    def parse_message(
        ...
        system_prompt = (
            ...
            "- אם המשתמש כותב משהו בסוגריים, זו הנחיה לסיווג אבל היא גם חלק מהתיאור.\n"
            '  למשל: "מונית 20 (טיסות)" — התיאור הוא "מונית (טיסות)", הסכום 20,\n'
            '  והסיווג צריך להתבסס על "טיסות".\n\n'
            "כלל קריטי — רשימת פריטים בשורה אחת:\n"
            "- אם שורה אחת מכילה סכום אחד ורשימת פריטים מופרדים בפסיקים או 'ו-' — זוהי הוצאה אחת בלבד.\n"
            '  למשל: "25.5 ביצים, נייר טואלט, אבקת כביסה וסבון" → הוצאה אחת, סכום 25.5.\n'
            "- אין לפצל רשימה כזו למספר הוצאות נפרדות.\n"
            "- התיאור יהיה הרשימה המלאה של הפריטים.\n"
            "- הקטגוריה תיקבע לפי הרוב: בחר את הקטגוריה שאליה שייכים הכי הרבה פריטים ברשימה.\n\n"
            "כלל קריטי — שמירת התיאור המקורי:\n"
            "- שדה התיאור (description) חייב לשקף את הניסוח המקורי של המשתמש.\n"
            "- הסר מהתיאור רק: סכום, מטבע, תאריך (אתמול/שלשום/dd-mm-yyyy), ופסיקים מיותרים.\n"
            "- אל תתרגם, תנסח מחדש, או תשנה את המילים שהמשתמש כתב.\n\n"
            f"קטגוריות אפשריות:\n{categories_block}\n\n"
            f"הנחיות סיווג:\n{directives_block}\n\n"
            f"מטבעות זמינים: {currencies_str}\n"
            f"מטבע ברירת מחדל: {default_currency}\n"
            f"אם המשתמש לא ציין מטבע בהודעה, חובה להשתמש במטבע ברירת המחדל ({default_currency}). "
            "אל תנחש מטבע לפי שפת ההודעה או הקשר אחר — תמיד השתמש במטבע ברירת המחדל.\n\n"
